...
!     Gravity model
!-------------------------------------------------
      character(len=4),parameter   :: tide_out='mean'    ! Required tide system
!-------------------------------------------------

!     Reference ellipsoid parameters
!     (Note routines assume normal earth ellipsoid)
!-------------------------------------------------
      !GRS80 (from ICGEM) 
      real*8,parameter :: a=6378137.0d0            ! Equatorial raidus (m)
      real*8,parameter :: f=1.0d0/298.257202101d0  ! Recipricol of flattening
      real*8,parameter :: gm=3.986005d14           ! Gravity mass constant
      real*8,parameter :: omega=7.292115d-5        ! Rotation rate
!-------------------------------------------------
!===========================================================
...
!     For geoid data
!-------------------------------------------------
      real*8,allocatable  :: c_n(:,:)     ! Sh coeffs for gravity model
      real*8,allocatable  :: s_n(:,:)           
      real*8,allocatable  :: shc(:,:,:)           
      real*8,allocatable  :: c_ref(:,:)   ! Ref ellipsiod sh coeffs
      real*8,allocatable  :: gh(:),lat(:),lon(:)
...
!-------------------------------------------------
      pin='./../a_mdt_data/src/'
      pout='./../a_mdt_data/computations/geoids/'
!-------------------------------------------------
...
!-------------------------------------------------
      ...
      allocate(lon(II))
      allocate(lat(JJ))
      allocate(c_n(0:NN_egm,0:NN_egm))
      allocate(s_n(0:NN_egm,0:NN_egm))
      allocate(c_ref(0:NN_egm,0:NN_egm))
!-------------------------------------------------
...
!     Read in EGM
!-------------------------------------------------
      allocate(shc(0:NN_egm,0:NN_egm,4))
      open(10,file=trim(pin)//trim(name_egm)//'.dat',form='unformatted')
      read(10)shc
      close(10)
      c_n(0:NN_egm,0:NN_egm)=shc(:,:,1)
      s_n(0:NN_egm,0:NN_egm)=shc(:,:,2)
      deallocate(shc)
      c_n(0,0)=0.0d0
!-------------------------------------------------

!     Convert tide system of geoid
!-------------------------------------------------
      call convert_tide(c_n(2,0),tide_in,tide_out)
!-------------------------------------------------
...
!     Compute geoid height on grid
!-------------------------------------------------
      do j=1,JJ

         write(*,*)'computing geoid for row',j

         call geoidh_mfc(a,f,gm,omega,NN,&
           &c_n(0:NN,0:NN),s_n(0:NN,0:NN),&
                     &II,lon,lat(j),gh)
...
!-------------------------------------------------
      ...
      fout=trim(name_egm)//'_do'//nn_str//'_rr'//rr_str//'.dat'
...
!===========================================================  
      ...
      subroutine geoidh_mfc(a,f,gm,omega,NN,c_n,s_n,&
                        &II,lon_d,lat_d,gh) 
!=====================================================================
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~     
!     Description:
!        Computes the geoid along a parallel using the 
...
!     Input variables
!-------------------------------------------------
      real*8,intent(in)  :: a                 ! Equatorial raidus
      real*8,intent(in)  :: f                 ! Recipricol of flattening
      real*8,intent(in)  :: gm                ! Gravity mass constant
      real*8,intent(in)  :: omega             ! Rotation rate
...
!     Calculate the normal gravity on the
!     surface of the ellipsoid
!-------------------------------------------------
      call norm_grav(a,f,gm,omega,cltgd,gamma)
!-------------------------------------------------

!     Compute the associated legendre functions
!     at required goecentric colatitude for
...
!-------------------------------------------------
      ...
      do i=1,2
         ...
         x2(:)=0.0d0
         do m=0,NN
            x1(m)=sum(rpwr(m:NN)*alf(m:NN,m)*c_n(m:NN,m))
            x2(m)=sum(rpwr(m:NN)*alf(m:NN,m)*s_n(m:NN,m))
